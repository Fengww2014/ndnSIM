...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  shared_ptr<lp::FwwTag> fwwTag = netPkt.getTag<lp::FwwTag>();
  if (fwwTag != nullptr) {
    lpPacket.add<lp::FwwTagField>(*fwwTag);
  }
  else {
    lpPacket.add<lp::FwwTagField>(0);
  }
  //Fww  
...
GenericLinkService::decodeInterest(const Block& netPkt, const lp::Packet& firstPkt)
{
 ...
 //Fww
  if (firstPkt.has<lp::FwwTagField>()) {
    interest->setTag(make_shared<lp::FwwTag>(firstPkt.get<lp::FwwTagField>()));
  }
 //Fww
...
//Fww
  if (firstPkt.has<lp::FwwTagField>()) {
    data->setTag(make_shared<lp::FwwTag>(firstPkt.get<lp::FwwTagField>() ));
  }
//Fww
