...
//#include <src/ndnSIM/ndn-cxx/src/mgmt/nfd/fib-entry.hpp>
#include "ns3/ndnSIM/model/ndn-l3-protocol.hpp" //bring in L3Protocol changed by fww
namespace nfd {
...
{
  //get node number in ndnsim changed by fww 11-4
  ...
  //fwwtag check to decide forward it or not changed by fww
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  if (pitMatches.size() == 0) {
    //fww
    std::cout<<"node-"<<node<<"-pit match-"<<data.getName()<<"-failed"<<std::endl;
...
*/
   ...
   if(ns3::Simulator::GetContext() == 0){
   ns3::Ptr<ns3::Node> thisNode;
   thisNode = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
   auto fibEntry_bool = thisNode->GetObject<ns3::ndn::L3Protocol>()->getForwarder()->getFib().insert(data.getName().getPrefix(1));
   fibEntry_bool.first->addNextHop(inFace,0,"node-1");
   const fib::NextHopList& nexthops = fibEntry_bool.first->getNextHops();
...
Forwarder::onOutgoingData(const Data& data, Face& outFace)
{
  ...
  if (fwwtag != nullptr )
    data.setTag(make_shared<lp::FwwTag> ( (*fwwtag)*10 + node) );
